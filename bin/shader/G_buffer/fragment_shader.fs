...
layout(location = 4) out vec4 ambient_buffer;
...
void main()
{
    ...
    vec2 texture_coord = get_parallax_mapping_texturecoord(fs_in.texture_coord, material, normalize(view_pos - vec3(fs_in.frag_position)));
    ...
    ambient_buffer = vec4(material.use_ambient_map ? texture(material.ambient_maps[0], texture_coord).rgb : material.ambient_color, 1.f);
}
