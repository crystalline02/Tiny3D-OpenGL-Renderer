...
vec4 caculatePBRColor();

void main()
{
    vec4 color = caculatePBRColor();

    // blendFunc: GL_ONE, GL_ONE
    // noice that we need to use gl_FragCoord.z as the depth of current transparent pixel(instead of the linear one between 0 and 1)
    float weight = clamp(pow(min(1.0, color.a * 10.0) + 0.01, 3.0) * 1e8 * pow(1.0 - gl_FragCoord.z * 0.9, 3.0), 1e-2, 3e3); 
    accum = vec4(color.rgb * color.a, color.a) * weight;  // result for render target 'accum'

    // blenFunc: GL_ZERO, GL_ONE_MINUS_SRC_COLOR
    revealage = color.a;  // result for render targer 'revealage'
}
...
float getAlpha()
{
    vec3 viewDir = normalize(view_pos - vec3(fsIn.fragPosition));
    vec2 textureCoord = get_parallax_mapping_texcoord(material, fsIn.textureCoord, viewDir);
    return material.use_opacity_map ? texture(material.opacity_maps[0], textureCoord).r : material.opacity;
}
...
vec3 cac_point_light(Point_light light, Material material)
{
    vec3 light_dir = normalize(light.position - vec3(fsIn.fragPosition));
    vec3 view_dir = normalize(view_pos - vec3(fsIn.fragPosition));
    vec2 texcoord = get_parallax_mapping_texcoord(material, fsIn.textureCoord, view_dir);
    ...
    if(dot(view_dir, normalize(fsIn.fragNormal)) < 0 || dot(light_dir, normalize(fsIn.fragNormal)) < 0) return vec3(0.f);

    float n_dot_l = max(dot(normal, light_dir), 0);  // cosTheta

    float r = length(light.position - vec3(fsIn.fragPosition));
    ...
    vec3 F = fresnelSchlick(F0, max(dot(normalize(fsIn.fragNormal), view_dir), 0.f));  // F
...
vec3 cac_env_irradiance(Skybox skybox, Material material)
{
    ...
    vec3 view_dir = normalize(view_pos - vec3(fsIn.fragPosition));
    vec2 tex_coord = get_parallax_mapping_texcoord(material, fsIn.textureCoord, view_dir);
    ...
    vec3 F = fresnelSchlickRoughness(F0, max(dot(fsIn.fragNormal, view_dir), 0.f), roughness);
...
vec3 get_shading_normal(Material material, vec2 texture_coord)
{
    vec3 normal = material.use_normal_map ? normalize(fsIn.TBN * (texture(material.normal_maps[0], texture_coord).rgb * 2 - 1)) : normalize(fsIn.fragNormal);
    if(material.use_normal_map)
        normal = normalize(mix(normalize(fsIn.fragNormal), normal, material.normal_map_strength));
    return normal;
}
...
vec2 get_parallax_mapping_texcoord(Material material, vec2 texcoord, vec3 view_dir)
{
    ...
    vec3 view_dir_tangent = normalize(transpose(fsIn.TBN) * view_dir);
...
float cac_point_shadow(Point_light light, vec3 light_dir)
{
    float bias_ = max(bias * (1 - dot(light_dir, normalize(fsIn.fragNormal))), bias * 0.01f);
    float in_shadow = 0.f;

    float radius = (1.f + linearize_depth(gl_FragCoord.z, near, far)) / 75.f;
    vec3 light2frag = vec3(fsIn.fragPosition) - light.position;
...
{
    float in_shadow = 0.f;
    float bias_ = max(bias * (1.f - dot(normalize(fsIn.fragNormal), normalize(-light.direction))), bias * 0.01f);
    ...
    vec4 light_space_clip = light_space_mat[index][shadow_index] * fsIn.fragPosition;
