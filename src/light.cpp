...
m_near(.1f), m_far(10.f), m_depth_map(-1), m_depth_fbo(-1)
{
    get_lights().emplace_back(this);
    m_depth_map_unit = Model::num_textures();
}
...
Light(positon, color, intensity, diffuse, specular), kc(1.0f), kl(0.35f), kq(0.44f)
{
    if(!flag)
    {
        ...
        m_index = c++;
        util::create_depthcubemap_framebuffer(m_depth_fbo, m_depth_map, m_depth_map_unit);
        Model::add_texture(("light_depth_map" + std::to_string(Light::lights_count())).c_str(), m_depth_map_unit);
    }
}
...
void Point_light::set_uniforms(const char* name, GLuint program) const
{
    ...
    util::set_int(light_depthcubemap.c_str(), m_depth_map_unit, program);
}

void Point_light::resize_depthmap(int shadow_size) const
{
    glActiveTexture(GL_TEXTURE0 + m_depth_map_unit);
...
Light(positon, color, intensity, diffuse, specular), m_direction(glm::normalize(direction))
{
    ...
    util::create_cascademap_framebuffer(m_depth_fbo, m_depth_map, m_depth_map_unit);
    Model::add_texture(("light_depth_map" + std::to_string(Light::lights_count())).c_str(), m_depth_map_unit);
}
...
void Direction_light::set_uniforms(const char* name, GLuint program) const
{
    ...
    util::set_int(light_cascade_maps.c_str(), m_depth_map_unit, program);
}
